...
from arekit.contrib.networks.context.configurations.att_sef_bilstm import AttentionSynonymEndsAndFramesBiLSTMConfig
from arekit.contrib.networks.context.architectures.att_sef_bilstm import AttentionSynonymEndsAndFramesBiLSTM
...
from rusentrel.classic.ctx.att_sef_bilstm import ctx_att_sef_bilstm_custom_config
from rusentrel.classic.common import \
    classic_common_callback_modification_func, \
    classic_mi_common_config_settings


def mi_att_sef_bilstm_custom_config(config):
    ctx_att_sef_bilstm_custom_config(config.ContextConfig)
...
def run_testing_att_sef_bilstm(name_prefix=u'',
    ...
    run_testing(full_model_name=name_prefix + model_names_classtype().AttSynonymEndsAndFramesBiLSTM,
                create_network=lambda: network_classtype(context_network=AttentionSynonymEndsAndFramesBiLSTM()),
                create_config=lambda: config_classtype(context_config=AttentionSynonymEndsAndFramesBiLSTMConfig()),
                cv_count=cv_count,
