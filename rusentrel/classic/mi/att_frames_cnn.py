...
from rusentrel.classic.ctx.att_frames_cnn import ctx_att_frames_cnn_custom_config
from rusentrel.classic.common import \
    classic_common_callback_modification_func, \
    classic_mi_common_config_settings


def mi_att_frames_cnn_config(config):
    ctx_att_frames_cnn_custom_config(config.ContextConfig)
...
def run_testing_att_frames_cnn(name_prefix=u'',
                               ...
                               custom_config_func=mi_att_frames_cnn_config,
                               custom_callback_func=classic_common_callback_modification_func):

    run_testing(full_model_name=name_prefix + model_names_classtype().AttFramesCNN,
                create_network=lambda: network_classtype(context_network=AttentionFramesCNN()),
                create_config=lambda: config_classtype(context_config=AttentionFramesCNNConfig()),
                cv_count=cv_count,
