...
from rusentrel.classic.ctx.att_ef_pcnn import ctx_att_ef_pcnn_custom_config
from rusentrel.classic.common import \
    classic_common_callback_modification_func, \
    classic_mi_common_config_settings


def mi_att_ef_pcnn_custom_config(config):
    ctx_att_ef_pcnn_custom_config(config.ContextConfig)
...
def run_testing_att_ef_pcnn(name_prefix=u'',
                            ...
                            custom_config_func=mi_att_ef_pcnn_custom_config,
                            custom_callback_func=classic_common_callback_modification_func):

    run_testing(full_model_name=name_prefix + model_names_classtype().AttEndsAndFramesPCNN,
                create_network=lambda: network_classtype(context_network=AttentionEndsAndFramesPCNN()),
                create_config=lambda: config_classtype(context_config=AttentionEndsAndFramesPCNNConfig()),
                cv_count=cv_count,
