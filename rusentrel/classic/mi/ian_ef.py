...
from rusentrel.classic.ctx.ian_ef import ctx_ian_ef_custom_config
...
def mi_ian_custom_config(config):
    ctx_ian_ef_custom_config(config.ContextConfig)
    config.fix_context_parameters()


def run_testing_ian_ef(name_prefix=u'',
                       cv_count=1,
    ...
    run_testing(full_model_name=name_prefix + model_names_classtype().IANEndsAndFrames,
                create_network=lambda: network_classtype(context_network=IANEndsAndFrames()),
                create_config=lambda: config_classtype(context_config=IANEndsAndFramesConfig()),
