...
from rusentrel.classic.ctx.ian_se import ctx_ian_se_custom_config
...
def mi_ian_custom_config(config):
    ctx_ian_se_custom_config(config.ContextConfig)
    config.fix_context_parameters()


def run_testing_ian_se(name_prefix=u'',
                         cv_count=1,
    ...
    run_testing(full_model_name=name_prefix + model_names_classtype().IANSynonymEnds,
                create_network=lambda: network_classtype(context_network=IANSynonymEndsBased()),
                create_config=lambda: config_classtype(context_config=IANSynonymEndsBasedConfig()),
