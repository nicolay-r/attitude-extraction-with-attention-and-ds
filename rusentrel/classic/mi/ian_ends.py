...
def mi_ian_custom_config(config):
    ctx_ian_ends_custom_config(config.ContextConfig)
    config.fix_context_parameters()


def run_testing_ian_ends(name_prefix=u'',
                         cv_count=1,
    ...
    run_testing(full_model_name=name_prefix + model_names_classtype().IANEnds,
                create_network=lambda: network_classtype(context_network=IANEndsBased()),
                create_config=lambda: config_classtype(context_config=IANEndsBasedConfig()),
