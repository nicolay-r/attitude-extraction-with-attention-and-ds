...
from rusentrel.classic.ctx.att_ef_bilstm import ctx_att_ef_bilstm_custom_config
from rusentrel.classic.common import \
    classic_common_callback_modification_func, \
    classic_mi_common_config_settings


def mi_att_ef_bilstm_custom_config(config):
    ctx_att_ef_bilstm_custom_config(config.ContextConfig)
...
def run_testing_att_ef_bilstm(name_prefix=u'',
                              ...
                              custom_config_func=mi_att_ef_bilstm_custom_config,
                              custom_callback_func=classic_common_callback_modification_func):

    run_testing(full_model_name=name_prefix + model_names_classtype().AttEndsAndFramesBiLSTM,
                create_network=lambda: network_classtype(context_network=AttentionEndsAndFramesBiLSTM()),
                create_config=lambda: config_classtype(context_config=AttentionEndsAndFramesBiLSTMConfig()),
                cv_count=cv_count,
