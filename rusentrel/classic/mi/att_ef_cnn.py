...
def mi_att_ef_cnn_custom_config(config):
    ctx_att_ef_cnn_custom_config(config.ContextConfig)
...
def run_testing_att_ef_cnn(name_prefix=u'',
                           ...
                           custom_config_func=mi_att_ef_cnn_custom_config,
                           custom_callback_func=classic_common_callback_modification_func):

    run_testing(full_model_name=name_prefix + model_names_classtype().AttEndsAndFramesCNN,
                create_network=lambda: network_classtype(context_network=AttentionEndsAndFramesCNN()),
                create_config=lambda: config_classtype(context_config=AttentionEndsAndFramesCNNConfig()),
                cv_count=cv_count,
